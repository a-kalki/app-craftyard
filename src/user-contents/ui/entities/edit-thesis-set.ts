...
type EditThesisSetType = EditThesisSetCommand['attrs'];

@customElement('thesis-set-edit-modal')
export class ThesisSetEditModal extends ValidatableElement<keyof EditThesisSetCommand['attrs']> {
  @property({ type: Object }) thesisSet!: ThesisSetAttrs;
  @property({ type: Boolean, reflect: true }) open = false;

  @state() private isLoading = false;
  @state() private formData: EditThesisSetType = {
    id: '',
    title: '',
    icon: '',
  };
  ...
  protected getFieldValue(field: keyof Omit<EditThesisSetType, 'id'>): unknown {
    return this.formData[field];
  }

  protected setFieldValue(field: keyof Omit<EditThesisSetType, 'id'>, value: unknown): void {
    this.formData = { ...this.formData, [field]: value };
  }

  async show(thesisSet: ThesisSetAttrs): Promise<boolean> {
    this.formData = { 
      id: thesisSet.id,
  ...
  saveable(): boolean {
    ...
    const valuesIsNotEqual = Object.entries(this.thesisSet)
      .some(([key, value]) => value !== this.formData[key as keyof EditThesisSetType])
  ...
  async save(): Promise<void> {
    ...
    try {
      const attrs: EditThesisSetCommand['attrs'] = {
        ...this.thesisSet,
        ...this.formData,
        ...(this.iconIsValid && { icon: this.formData.icon })
      };

      const updateResult = await this.thesisSetApi.editThesisSet(attrs);
...
declare global {
  interface HTMLElementTagNameMap {
    'thesis-set-edit-modal': ThesisSetEditModal;
  }
}
