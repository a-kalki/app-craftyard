...
export class EditThesisModal extends ValidatableElement<keyof EditThesistAttrs> {
  static styles = css`
  ...
  @state() private formData!: EditThesistAttrs;
  protected iconIsValid = true;
  private resolve?: (value: 'success' | null) => void;

  async show(thesis: ThesisContent): Promise<'success' | null> {
    this.thesis = {...thesis};
  ...
  private hasChanged(): boolean {
    ...
    const valuesIsNotEqual = Object.entries(this.thesis)
      .some(([key, value]) => value !== this.formData[key as keyof EditThesistAttrs])
  ...
  private async save() {
    ...
    try {
      ...
      const updateResult = await this.userContentApi.editContent(this.formData);
  ...
  protected getFieldValue(field: keyof EditThesistAttrs): unknown {
    return this.formData[field];
  }

  protected setFieldValue(field: keyof EditThesistAttrs, value: unknown): void {
