...
export class ThesisCard extends BaseElement {
  ...
  private async confirmDelete() {
    ...
    try {
      ...
      if (confirmed) {
        const commandAttrs: DeleteThesisCommand['attrs'] = {
          id: this.thesisSetId,
          thesisId: this.thesis.id,
        }
        const result = await this.thesisSetApi.deleteThesis(commandAttrs)
        if (result.isFailure()) {
          this.app.error(
  ...
  private async handleEditThesis(): Promise<void> {
    ...
    try {
      const result = await modal.show(this.thesisSetId, this.thesis);
      if (!result) return;
  ...
  private thesisSetEditedEvent(): void {
    const event: CustomEvent<{ id: string }> = new CustomEvent(
      'thesis-set-edited',
      { detail: { id: this.thesisSetId }, bubbles: true, composed: true }
    );
